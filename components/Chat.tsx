    ...
    <div className="max-w-6xl mx-auto space-y-4">
      <div className="flex flex-col space-y-4 border-b border-gray-700 pb-4">
        ...
        <div className="text-white flex items-center gap-2">
          <p className="text-2xl whitespace-nowrap font-semibold -translate-y-0.25">
            Type 🦝{" "}
          </p>
          <WordRotate
            words={[
              '"Show me <span class="text-gray-500">biology</span> classes that start at <span class="text-gray-500">1pm</span> on <span class="text-gray-500">Fridays</span>"',
              '"Find <span class="text-gray-500">large</span> <span class="text-gray-500">psychology</span> lectures that start in the <span class="text-gray-500">afternoon</span>"',
              '"I am interested in <span class="text-gray-500">AI</span> and <span class="text-gray-500">machine learning</span>"',
              '"What is the <span class="text-gray-500">biggest</span> <span class="text-gray-500">music</span> class?"',
              '"I like <span class="text-gray-500">True Crime</span>, what courses should I go to?"',
            ]}
      ...
      <div className="bg-gray-800/50 backdrop-blur rounded-lg p-6 min-h-[300px] flex flex-col">
        <div className="flex-1 space-y-4 mb-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
          {messages.length === 0 ? (
            <div className="text-white text-center py-4 flex flex-col items-center gap-4">
              <BotMessageSquare className="w-20 h-20 text-white opacity-50" />
            ...
            messages.map((message, index) => (
              ...
              >
                ...
                >
                  {message.role === "user" ? (
                    <div className="text-4xl mt-4 -translate-x-1 -translate-y-0.5">
                      🦝
                    </div>
                  ) : (
                                ...
                                <div className="bg-gray-800/50 rounded-lg p-3 shadow-lg border border-gray-700">
                                  ...
                                  <div className="flex items-center gap-3 text-sm mb-2 bg-[#32405a] rounded-lg px-3 py-2 shadow-inner">
                                    <span className="text-sm font-medium flex items-center gap-1">
                                      ...
                                      <span className="text-white">
                                        {details
                                          .find((d) => d.label === "Schedule")
                                          ?.value?.replace("Meets on ", "")}
                                      </span>
                                    </span>
                                  ...
                                  <div className="grid grid-cols-1 gap-2">
                                    {details
                                      ...
                                      .map((detail, i) => (
                                        <div key={i}>
                                          <span className="block text-blue-300/90 text-sm font-medium mb-0.5">
                                            {detail.label}
                                          </span>
                                          <span className="block text-white text-sm leading-snug">
                                            {detail.value}
                                          </span>
                                        </div>
                                      ))}
            ...
            messages[messages.length - 1].role === "user" && (
              <div className="flex items-start gap-3">
                <div className="shrink-0 mr-2">
                  <BotMessageSquare className="h-8 w-8 text-white animate-bounce" />
                </div>
                <div className="relative rounded-lg px-4 py-2 bg-gray-700/50 text-white shadow-[0_4px_10px_rgba(0,0,0,0.5)]">
                  Thinking...
                </div>
              </div>
            )}
        ...
        <form onSubmit={handleSubmit} className="flex gap-2 relative">
          <input
            ...
            className="flex-1 bg-gray-700/50 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 pr-10"
            disabled={isLoading}
          />
          <button
            type="submit"
