...
export default function LectureList({
  ...
  const [currentTime, setCurrentTime] = useState(formatCSTTime());
  const [selectedSubject, setSelectedSubject] = useState<string>("");
  const [selectedUpcomingSubject, setSelectedUpcomingSubject] =
    useState<string>("");
  const [selectedTimeFrame, setSelectedTimeFrame] = useState<number>(60); // Default 1 hour (60 minutes)
  const [selectedCatalogSubject, setSelectedCatalogSubject] =
    useState<string>("");
  ...
  const [courseDescriptions, setCourseDescriptions] = useState<
    Record<string, CourseDescription>
  >({});
  ...
  const filteredLiveClasses = useMemo(() => {
    return uniqueClasses
      .filter((c) => {
        ...
        return (
          isLive &&
          (!selectedSubject || c.courseCode.startsWith(selectedSubject))
        );
      })
      .sort((a, b) => {
        if (sortByRecent) {
          ...
          return (
            parseTime(startTimeB).getTime() - parseTime(startTimeA).getTime()
          );
        } else {
    ...
    return uniqueClasses
      .filter((c) => {
        ...
        const minutesUntilStart =
          (startDate.getTime() - new Date().getTime()) / 60000;
        return (
          isClassDay(c.days) &&
          minutesUntilStart > 0 &&
          minutesUntilStart <= selectedTimeFrame
        );
      })
  ...
  const catalogSubjectsCount = uniqueClasses
    .filter((c) => {
      if (
        ...
        !c.courseCode.startsWith(selectedCatalogSubject)
      )
        return false;
      if (selectedCatalogDay && !c.days.includes(selectedCatalogDay))
        return false;
      return true;
    })
    .reduce((subjects, c) => {
  ...
  const filteredUpcomingClasses = useMemo(() => {
    return selectedUpcomingSubject
      ? upcomingClasses.filter((c) =>
          c.courseCode.startsWith(selectedUpcomingSubject)
        )
      : upcomingClasses;
  ...
  const renderTableHeader = (mode: "live" | "upcoming" | "catalog") => (
    <div
      className={`grid ${
        mode === "live"
          ? "grid-cols-[2fr,1.5fr,0.7fr,0.7fr,0.5fr,1fr,0.7fr]"
          : mode === "upcoming"
          ? "grid-cols-[2fr,1.5fr,0.7fr,0.7fr,0.5fr,1fr,0.7fr]"
          : "grid-cols-[2fr,1.5fr,0.7fr,0.7fr,0.5fr,0.5fr,1fr]"
      } gap-x-2 px-6 py-3 bg-gray-700 text-xs font-medium ${
        mode === "live" ? "text-white" : "text-white"
      ...
      {(mode === "live" || mode === "upcoming") && (
        <div
          className={`text-right pr-2 ${
            mode === "upcoming" ? "text-white" : "text-white"
          }`}
        >
  ...
  const renderClassRow = (
    classItem: ClassItem,
    status: "live" | "upcoming" | "catalog",
    isLast: boolean
  ) => {
    const dotColor =
      status === "live"
        ? "bg-green-500 shadow-green-500/75 shadow-[0_0_5px_3px]"
        : "bg-gray-500";
    ...
    const cellClass = `flex items-center min-h-full gap-2 ${
      status === "live" ? "text-white font-medium" : "text-white"
    }`;
    const iconClass = `w-5 h-5 ${
      status === "live" ? "text-white" : "text-white"
    } flex-shrink-0`;

    // Get course description if available
    const descriptionKey = classItem.courseCode.split(" ")[1]; // "MUGC 5210" → "5210"
    const description = courseDescriptions[descriptionKey] || null;

    console.log("🧪 Looking for description:", {
      courseCode: classItem.courseCode,
      normalizedKey: descriptionKey,
      found: courseDescriptions[descriptionKey],
    });
      ...
      <div
        ...
        className={`grid ${
          status === "live"
            ...
            ? "grid-cols-[2fr,1.5fr,0.7fr,0.7fr,0.5fr,1fr,0.7fr]"
            : "grid-cols-[2fr,1.5fr,0.7fr,0.7fr,0.5fr,0.5fr,1fr]"
        } gap-x-2 px-6 py-4 text-sm hover:bg-gray-700/50 ${
      ...
      >
        <div className={cellClass}>
          <div className="flex items-center gap-3">
            <div
              className={`w-2 h-2 rounded-full ${dotColor} ${dotAnimation} flex-shrink-0`}
            />
          ...
          <div className={`${cellClass} justify-end pr-2`}>
            ...
            <span
              className={`whitespace-nowrap ${
                status === "live" ? timeColor : "text-blue-400"
              }`}
            >
              {status === "live"
  ...
  const catalogClasses = useMemo(() => {
    const filtered = uniqueClasses
      .filter((classItem) => {
        if (
          selectedCatalogSubject &&
          !classItem.courseCode.startsWith(selectedCatalogSubject)
        )
          return false;
        if (selectedCatalogDay && !classItem.days.includes(selectedCatalogDay))
          return false;
        return true;
      })
      .sort((a, b) => {
    ...
    return (
      <div className="space-y-8">
        <div className="border-b border-gray-700 pb-4">
          <h1 className="text-4xl font-bold text-white mb-4 py-4">
            Course Catalog (...give it a second)
          </h1>
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() =>
                    setSelectedCatalogDay(selectedCatalogDay === "M" ? "" : "M")
                  }
                ...
                <button
                  onClick={() =>
                    setSelectedCatalogDay(
                      selectedCatalogDay === "Tu" ? "" : "Tu"
                    )
                  }
                  className={`px-3 py-1.5 rounded ${
                ...
                <button
                  onClick={() =>
                    setSelectedCatalogDay(selectedCatalogDay === "W" ? "" : "W")
                  }
                  className={`px-3 py-1.5 rounded ${
                ...
                <button
                  onClick={() =>
                    setSelectedCatalogDay(selectedCatalogDay === "R" ? "" : "R")
                  }
                  className={`px-3 py-1.5 rounded ${
                    selectedCatalogDay === "R"
                      ? "bg-gray-700 text-white shadow-[0_0_12px_-2px_rgba(0,0,0,0.7)]"
                      : "text-white hover:text-white"
                  }`}
                >
                ...
                <button
                  onClick={() =>
                    setSelectedCatalogDay(selectedCatalogDay === "F" ? "" : "F")
                  }
                  className={`px-3 py-1.5 rounded ${
              ...
              <div className="flex items-center">
                <p className="text-white translate-x-4">
                  There are{" "}
                  <span className="text-white">{totalFilteredClasses}</span>{" "}
                  courses
                  {selectedCatalogSubject ? ` in ` : ` across `}
                  <span className="text-white">
                    {selectedCatalogSubject || catalogSubjectsCount}
                  </span>{" "}
                  subjects
                  {selectedCatalogDay && (
                    <>
                ...
                {catalogClasses.length > 0 && (
                  <div
                    className={`flex items-center gap-2 ml-auto ${
                      selectedCatalogDay ? "translate-x-16" : "translate-x-44"
                    }`}
                  >
                    <button
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex items-center justify-between gap-4 mt-2">
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 text-sm text-white bg-gray-700 rounded-md border border-gray-600 hover:border-yellow-500 transition-colors shadow-[0_0_15px_-3px_rgba(0,0,0)]"
                title={
                  sortByRecent
            ...
            <div className="text-xl font-mono text-white-300 ml-auto">
      ...
      <div>
        <div className="border-b border-gray-700 pb-4">
          <h2 className="text-2xl font-bold text-white">Starting Soon</h2>
          <div className="flex items-center justify-between gap-4 mt-2">
            <div className="flex items-center gap-4">
              <div className="w-96">
                <select
              ...
              <p className="text-white">
                {filteredUpcomingClasses.length === 0 ? (
                  <>
                    No classes starting in the next{" "}
                    <span className="text-white">
                      {selectedTimeFrame === 60
                        ? "1h"
                        : selectedTimeFrame === 30
            ...
            <div className="bg-gray-800 rounded-lg overflow-hidden shadow-[0_0_12px_-3px_rgba(55,64,81,0.7)]">
              ...
              {filteredUpcomingClasses.map((c, i) =>
                renderClassRow(
                  c,
                  "upcoming",
                  i === filteredUpcomingClasses.length - 1
                )
              )}
            </div>
          </div>
